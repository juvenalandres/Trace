<script lang="ts">
  ...
  const stepColors: Record<string, string> = {
    warmup: 'var(--primary)', interval: 'var(--danger)', recovery: 'var(--success)',
    cooldown: 'var(--primary)', rest: 'var(--text-tertiary)', other: '#8b5cf6',
  };
...
<style>
  .ws { margin-top: 12px; padding: 12px 14px; background: var(--bg); border: 0.5px solid var(--border); border-radius: var(--radius-md, 8px); }
  .ws-h { font-size: var(--font-size-xs, 11px); font-weight: var(--font-weight-semibold, 600); text-transform: uppercase; letter-spacing: 0.5px; color: var(--text-secondary); margin-bottom: 8px; }
  .ws-grid { display: flex; flex-wrap: wrap; gap: 16px; }
  .ws-stat { display: flex; flex-direction: column; gap: 1px; }
  .ws-v { font-size: var(--font-size-lg, 16px); font-weight: var(--font-weight-semibold, 600); color: var(--text); line-height: 1.2; }
