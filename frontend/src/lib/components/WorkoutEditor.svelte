        ...
        <div class="we-block">
          <div class="we-bh">
            ...
            <div class="we-ba-acts">
              <button class="icon-btn" onclick={() => copyBlock(bi)} title="Copy block">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 01-2-2V4a2 2 0 012-2h9a2 2 0 012 2v1"/></svg>
              </button>
              <button class="icon-btn danger" onclick={() => remBlock(bi)}>
          ...
          <div class="we-steps">
            {#each block.steps as step, si}
              {@const colors = { warmup: 'var(--primary)', interval: 'var(--danger)', recovery: 'var(--success)', cooldown: 'var(--primary)', rest: 'var(--text-tertiary)', other: '#8b5cf6', repeat: 'var(--warning)' } as Record<string,string>}
              ...
              <div class="we-step" draggable="true" style="--sc: {sc}"
                ...
                <div class="we-sh">
                  ...
                  {/if}
                  <button class="icon-btn" onclick={() => copyStep(bi, si)} title="Copy step">
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 01-2-2V4a2 2 0 012-2h9a2 2 0 012 2v1"/></svg>
                  </button>
                  {#if clipboard?.type === 'step'}
                    <button class="icon-btn" onclick={() => pasteStep(bi, si)} title="Paste after">
                  ...
                  {/if}
                  <button class="icon-btn danger" onclick={() => remStep(bi, si)}>
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
                  </button>
                </div>
                    ...
                    {#each step.steps as ns, ni}
                      ...
                      <div class="we-step" style="--sc: {nsc}">
                        <div class="we-sh">
                          ...
                          <button class="icon-btn danger" onclick={() => { const ss = (step.steps ?? []).filter((_, j) => j !== ni); patchSteps(bi, si, ss); }}>
                            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
                          </button>
                        </div>
                        <div class="we-ctrl">
                        ...
                        <div class="we-ctrl">
                          ...
                          {#if ns.target}
                            ...
                            {/if}
                            <button class="we-clr" onclick={() => patchNested(bi, si, ni, { target: null })}>
                ...
                {:else}
                  ...
                  <div class="we-ctrl">
                    ...
                    {#if step.target}
                      ...
                      {/if}
                      <button class="we-clr" onclick={() => patchStep(bi, si, { target: null })}>
                        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
                      </button>
                    {/if}
                  </div>
...
<style>
  ...
  .btn { display: inline-flex; align-items: center; gap: 6px; padding: 8px 16px; border: none; border-radius: var(--radius-md, 8px); font-family: var(--font-sans); font-size: var(--font-size-base, 13px); font-weight: var(--font-weight-medium, 500); cursor: pointer; transition: background var(--transition-fast), color var(--transition-fast); }
  ...
  .btn-xs { padding: 4px 10px; font-size: var(--font-size-xs, 11px); border-radius: var(--radius-md, 6px); }

  .we-blocks { display: flex; flex-direction: column; gap: 14px; }
  .we-block { border: 0.5px solid var(--border); border-radius: var(--card-radius, 10px); padding: 14px; border-left: 3px solid var(--primary); background: var(--surface); transition: box-shadow var(--transition-base); }
  .we-block:hover { box-shadow: var(--shadow-sm); }
  ...
  .we-bn { flex: 1; font-size: var(--font-size-md, 14px); font-weight: var(--font-weight-semibold, 600); border: none; background: transparent; color: var(--text); outline: none; padding: 2px 4px; border-bottom: 1px solid transparent; transition: border-color var(--transition-fast); }
  .we-bn:focus { border-bottom-color: var(--primary); }
  .we-steps { display: flex; flex-direction: column; gap: 8px; }
  .we-step { border: 0.5px solid var(--border); border-left: 3px solid var(--sc, var(--primary)); border-radius: var(--radius-md, 8px); padding: 10px 12px; background: var(--bg); transition: box-shadow var(--transition-fast); }
  .we-step:hover { box-shadow: var(--shadow-sm); }
  .we-sh { display: flex; align-items: center; gap: 8px; flex-wrap: wrap; }
  .we-drag { cursor: grab; color: var(--text-secondary); user-select: none; opacity: 0.4; transition: opacity var(--transition-fast); }
  .we-drag:hover { opacity: 0.7; }
  .we-sel { font-size: var(--font-size-sm, 12px); font-weight: var(--font-weight-semibold, 600); padding: 4px 8px; border: 0.5px solid var(--border); border-radius: var(--radius-md, 6px); background: var(--surface); color: var(--text); outline: none; cursor: pointer; transition: border-color var(--transition-fast); }
  .we-sel:hover { border-color: var(--primary); }
  .we-sbtn { padding: 4px 10px; font-size: var(--font-size-xs, 11px); font-weight: var(--font-weight-medium, 500); border: 0.5px solid var(--border); border-radius: var(--radius-md, 6px); background: var(--surface); color: var(--text-secondary); cursor: pointer; transition: all var(--transition-fast); }
  ...
  .we-rep input { width: 36px; padding: 3px 4px; font-size: var(--font-size-sm, 12px); text-align: center; border: 0.5px solid var(--border); border-radius: var(--radius-md, 4px); background: var(--surface); color: var(--text); }
  ...
  .we-cl { font-size: var(--font-size-xs, 11px); color: var(--text-secondary); white-space: nowrap; font-weight: var(--font-weight-medium, 500); text-transform: uppercase; letter-spacing: 0.3px; }
  .we-inp { width: 52px; padding: 4px 6px; font-size: var(--font-size-sm, 12px); border: 0.5px solid var(--border); border-radius: var(--radius-md, 6px); background: var(--surface); color: var(--text); transition: border-color var(--transition-fast); }
  .we-inp:focus { outline: none; border-color: var(--primary); }
  .we-inp[type="text"] { width: 80px; }
  .we-un { font-size: var(--font-size-xs, 11px); padding: 4px 4px; border: 0.5px solid transparent; border-radius: var(--radius-md, 4px); background: transparent; color: var(--text-secondary); cursor: pointer; transition: all var(--transition-fast); }
  ...
  .we-tb { padding: 4px 10px; font-size: var(--font-size-xs, 11px); font-weight: var(--font-weight-medium, 500); border: 0.5px solid var(--border); border-radius: var(--radius-md, 6px); background: var(--surface); color: var(--text-secondary); cursor: pointer; white-space: nowrap; transition: all var(--transition-fast); }
  .we-tb:hover { border-color: var(--primary); }
  .we-tb.a { border-color: var(--primary); color: var(--primary); background: var(--primary-light); }
  .we-cy { font-size: var(--font-size-sm, 12px); border: none; background: transparent; color: var(--text-secondary); cursor: pointer; padding: 2px 4px; border-radius: var(--radius-md, 4px); transition: background var(--transition-fast); }
  .we-cy:hover { background: var(--hover); }
  .we-clr { border: none; background: transparent; color: var(--danger); cursor: pointer; padding: 2px 6px; border-radius: var(--radius-md, 4px); display: inline-flex; align-items: center; transition: background var(--transition-fast); }
  .we-clr:hover { background: var(--danger-bg); }
  .icon-btn { width: 32px; height: 32px; border: none; background: none; color: var(--text-secondary); cursor: pointer; border-radius: var(--radius-md, 6px); display: flex; align-items: center; justify-content: center; transition: all var(--transition-fast); }
  ...
  .we-chart { margin-top: 14px; border-radius: var(--radius-md, 8px); overflow: hidden; border: 0.5px solid var(--border); background: var(--bg); }
